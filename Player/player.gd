...
const SECOND_JUMP_VELOCITY = 4.0
const MAX_JUMPS = 2 
const DECAY := 8.0
...
@export var stats: character_status
...
func _physics_process(delta: float) -> void:
	...
	if Input.is_action_just_pressed("ui_accept") and jumps_left > 0:
		if is_on_floor():
			velocity.y = JUMP_VELOCITY + stats.get_base_speed()
		else:
			velocity.y = SECOND_JUMP_VELOCITY + stats.get_base_speed()
		jumps_left -= 1
